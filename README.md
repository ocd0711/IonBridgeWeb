...
不带协议时会自动补成 `http://`。点击 `保存并连接` 后，服务端会等待目标连接完成并读取 PSN。只有成功拿到 PSN 后，目标才会成为已保存设备；失败时按钮会恢复可点击，并显示错误。
...
地址右侧的数字输入框是当前设备的 metrics 获取频率，单位为秒。默认是 `30s`，允许范围是 `1s` 到 `60s`。目标连接成功并拿到 PSN 后立即生效。
